...
# compiled version — compiled once at module level
compiled_attention = torch.compile(attention)


def benchmark_attention(d_model, seq_len, device="cuda", compiled=False):
    use_cuda = device == "cuda" and torch.cuda.is_available()
    attn_fn = compiled_attention if compiled else attention
...
def main():
    ...
    parser.add_argument(
        ...
        help="Benchmark torch.compile(attention) instead of eager attention",
    )
    ...
    print(f"{'='*50}")
    print(f"  Attention benchmark  mode={mode}")
    print(f"{'='*50}")

    results = []
    for d_model, seq_len in product(D_MODELS, SEQ_LENS):
        print(f"d_model={d_model:4d}  seq_len={seq_len:6d} ... ", end="", flush=True)
        row = benchmark_attention(d_model, seq_len, args.device, compiled=args.compiled)
        if row is None:
